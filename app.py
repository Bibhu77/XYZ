...
TWILIO_TRIAL_MODE = True  # Set to True for trial account, False for paid account
...
def validate_phone_number(phone):
    ...
    if not phone:
        return False
    # Convert to string to handle bytes-like objects
    phone_str = str(phone)
...
def match():
    ...
    try:
        ...
        if matches and twilio_client:
            ...
            # Debug: Log phone number and type
            logging.debug(f"Phone number before validation: {to_phone} (type: {type(to_phone)})")
            ...
            if not validate_phone_number(to_phone):
                ...
                # In trial mode, mock SMS or use a verified number
                sms_status = "mocked: trial mode"
                logging.info(f"Mocked SMS to {to_phone} due to trial mode")
                # For testing, you can set to_phone to a verified number
                # to_phone = "+919876543210"  # Replace with your verified number
            else:
                ...
                try:
                    logging.info(f"Attempting to send SMS to {to_phone}")
                    ...
                    if "21211" in str(sms_error):
                        sms_status = f"failed: Invalid 'To' phone number: {to_phone}. See https://www.twilio.com/docs/errors/21211"
